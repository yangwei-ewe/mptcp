...
namespace ns3 {
    ...
    MpTcpSubFlow::MpTcpSubFlow() :
        ...
        NS_ASSERT(rtt->GetEstimate().GetMicroSeconds() != 0);
        //   rtt->Gain(0.1);
        // Time estimate;
    ...
    bool MpTcpSubFlow::Finished(void) {
    ...
    void MpTcpSubFlow::RttSent(SequenceNumber32 seq, bool isRetransmission) {
        if (isRetransmission) {
            return;
        }

        // 只追蹤第一個 outstanding packet
        if (!m_rttTracking) {
            m_rttTracking = true;
            m_rttSeq = seq;
            m_rttSendTime = Simulator::Now();
        }
    }

    Time MpTcpSubFlow::RttAcked(SequenceNumber32 ackSeq) {
        Time sample = Seconds(0);
