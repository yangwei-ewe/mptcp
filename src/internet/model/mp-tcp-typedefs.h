...
#define ASSERT(cond, error_msg)                                                                    \
    ...
    inline void
        set_bit8(uint8_t& flag, uint8_t offset) {
        (flag) |= (1U << (offset));
        return;
    }

    inline bool
        get_bit8(uint8_t& flag, uint8_t offset) {
        return (1U & (flag >> offset));
    }

    inline void
        clear_bit8(uint8_t& flag, uint8_t offset) {
        flag &= ~(1U << (offset));
        return;
    }
    ...
    typedef enum Reason : uint8_t {                                       // Reset reason for MP_TCPRST
    ...
    enum MP_SubType {                       // the MPTCP option subtype codes defined by rfc 8684
    ...
    public:
        ...
        void Print(std::ostream& os) const override {
            os << "MPTCP Option: Subtype=" << +this->GetSubType() << ", Kind=" << +this->GetKind()
        ...
        bool is_known_subtype(uint8_t subtype);
 ...
 * C: set to 1 to indicate that the sender of this option will not accept additional MPTCP subflows
 * to the source address and port H: use of HMAC-SHA256. An implementation that only supports this
 * method MUST set bit "H" to 1 and bits "D" through "G" to 0.Furthermore, if D~H are all 0, the
 * MP_CAPABLE should be treated as invalid and ignored(i.e. back to TCP handshake)
 */
    ...
    public:
        ...
        typedef enum checksum_algo : uint8_t { // represent offset here
            HMAC_SHA256 = 0
        } checksum_algo_t;
        ...
        checksum_algo_t get_checksum_algo() const {
            return checksum_algo;
        }

        void set_checksum_algo(checksum_algo_t algo) {
            checksum_algo = algo;
        }
...
#define MP_JOIN_FLAG_B 0
     ...
     * Length, and Checksum (if negotiated) present a (Data ACK length): Data ACK is 8 octets (if not
     * set, Data ACK is 4 octets) A (Data ACK present): Data ACK present
     */
