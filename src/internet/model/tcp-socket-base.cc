...
namespace {
    ...
    /**
     * @brief map TcpPacketType and EcnMode to boolean value to check whether ECN-marking is allowed
     * or not
     */
    const std::map<std::pair<ns3::TcpSocketBase::TcpPacketType_t, ns3::TcpSocketState::EcnMode_t>,
                   bool>
        ECN_RESTRICTION_MAP{
...
namespace ns3 {
    ...
    TypeId TcpSocketBase::GetTypeId() {
        static TypeId tid =
    ...
    TcpSocketBase::TcpSocketBase()
        ...
        ok =
            m_tcb->TraceConnectWithoutContext("HighestSequence",
        ...
        ok =
            m_tcb->TraceConnectWithoutContext("RTT", MakeCallback(&TcpSocketBase::UpdateRtt, this));
        NS_ASSERT(ok == true);
    ...
    Socket::SocketErrno TcpSocketBase::GetErrno() const {
        return m_errno;
    }

    /* Inherit from Socket class: Returns socket type, NS3_SOCK_STREAM */
    Socket::SocketType TcpSocketBase::GetSocketType() const {
        return NS3_SOCK_STREAM;
    }

    /* Inherit from Socket class: Returns associated node */
    Ptr<Node> TcpSocketBase::GetNode() const {
        return m_node;
    }

    /* Inherit from Socket class: Bind socket to an end-point in TcpL4Protocol */
    int TcpSocketBase::Bind() {
        NS_LOG_FUNCTION(this);
    ...
    void TcpSocketBase::SetInitialSSThresh(uint32_t threshold) {
        NS_ABORT_MSG_UNLESS((m_state == CLOSED) || threshold == m_tcb->m_initialSsThresh,
                            "TcpSocketBase::SetSSThresh() cannot change initial ssThresh after "
                            "connection started.");

        m_tcb->m_initialSsThresh = threshold;
    }

    uint32_t TcpSocketBase::GetInitialSSThresh() const {
    ...
    uint32_t TcpSocketBase::GetInitialCwnd() const {
    ...
    int TcpSocketBase::Connect(const Address& address) {
        ...
        if (InetSocketAddress::IsMatchingType(address)) {
            ...
            // Get the appropriate local address and port number from the routing protocol and set
            // up endpoint
            if (SetupEndpoint() != 0) {
    ...
    int TcpSocketBase::Close() {
        ...
        if (m_tcb->m_rxBuffer->Size() != 0) {
            NS_LOG_WARN(
                "Socket " << this << " << unread rx data during close.  Sending reset."
                          << "This is probably due to a bad sink application; check its code");
            SendRST();
            return 0;
        }

        if (m_txBuffer->SizeFromSequence(m_tcb->m_nextTxSequence) >
            0) { // App close with pending data must wait until all data transmitted
            if (!m_closeOnEmpty) {
                m_closeOnEmpty = true;
    ...
    int TcpSocketBase::ShutdownSend() {
        NS_LOG_FUNCTION(this);
        ...
        if (m_txBuffer->Size() == 0) {
            if (m_state == ESTABLISHED || m_state == CLOSE_WAIT) {
                ...
                if (m_state == ESTABLISHED) { // On active close: I am the first one to send FIN
                    NS_LOG_DEBUG("ESTABLISHED -> FIN_WAIT_1");
                    m_state = FIN_WAIT_1;
                } else { // On passive close: Peer sent me FIN already
    ...
    int TcpSocketBase::ShutdownRecv() {
        NS_LOG_FUNCTION(this);
        m_shutdownRecv = true;
        return 0;
    }
    ...
    int TcpSocketBase::Send(Ptr<Packet> p, uint32_t flags) {
        ...
        if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT) {
            ...
            if (!m_txBuffer->Add(p)) { // TxBuffer overflow, send failed
            ...
            if ((m_state == ESTABLISHED || m_state == CLOSE_WAIT) &&
                AvailableWindow() >
                    0) { // Try to send the data out: Add a little step to allow the application
                // to fill the buffer
    ...
    uint32_t TcpSocketBase::GetTxAvailable() const {
        NS_LOG_FUNCTION(this);
        return m_txBuffer->Available();
    }

    /* Inherit from Socket class: Get the max number of bytes an app can read */
    uint32_t TcpSocketBase::GetRxAvailable() const {
        NS_LOG_FUNCTION(this);
    ...
    int TcpSocketBase::GetSockName(Address& address) const {
        ...
        if (m_endPoint != nullptr) {
            ...
            address =
                Inet6SocketAddress(m_endPoint6->GetLocalAddress(), m_endPoint6->GetLocalPort());
    ...
    int TcpSocketBase::SetupCallback() {
        NS_LOG_FUNCTION(this);
    ...
    int TcpSocketBase::DoConnect() {
        NS_LOG_FUNCTION(this);

        // A new connection is allowed only if this socket does not have a connection
        if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK ||
            m_state == CLOSE_WAIT) { // send a SYN packet and change state into SYN_SENT
            ...
            m_state = SYN_SENT;
            m_tcb->m_ecnState =
                TcpSocketState::ECN_DISABLED; // because sender is not yet aware about
            // receiver's ECN capability
        } else if (m_state != TIME_WAIT) { // In states SYN_RCVD, ESTABLISHED, FIN_WAIT_1,
    ...
    void TcpSocketBase::CloseAndNotify() {
        NS_LOG_FUNCTION(this);
    ...
    bool TcpSocketBase::OutOfRange(SequenceNumber32 head, SequenceNumber32 tail) const {
        if (m_state == LISTEN || m_state == SYN_SENT ||
            m_state == SYN_RCVD) { // Rx buffer in these states are not initialized.
            return false;
        }
        if (m_state == LAST_ACK || m_state == CLOSING ||
            m_state ==
                CLOSE_WAIT) { // In LAST_ACK and CLOSING states, it only wait for an ACK and the
    ...
    void TcpSocketBase::ForwardUp(Ptr<Packet> packet,
                                  Ipv4Header header,
                                  uint16_t port,
                                  Ptr<Ipv4Interface> incomingInterface) {
        NS_LOG_LOGIC("Socket " << this << " forward up " << m_endPoint->GetPeerAddress() << ":"
                               << m_endPoint->GetPeerPort() << " to "
    ...
    void TcpSocketBase::ForwardUp6(Ptr<Packet> packet,
                                   Ipv6Header header,
                                   uint16_t port,
        ...
        Address toAddress =
            Inet6SocketAddress(header.GetDestination(), m_endPoint6->GetLocalPort());
    ...
    void TcpSocketBase::ForwardIcmp(Ipv4Address icmpSource,
                                    uint8_t icmpTtl,
                                    uint8_t icmpType,
                                    uint8_t icmpCode,
                                    uint32_t icmpInfo) {
    ...
    void TcpSocketBase::ForwardIcmp6(Ipv6Address icmpSource,
                                     uint8_t icmpTtl,
                                     uint8_t icmpType,
                                     uint8_t icmpCode,
                                     uint32_t icmpInfo) {
    ...
    bool TcpSocketBase::IsValidTcpSegment(const SequenceNumber32 seq,
    ...
    void TcpSocketBase::DoForwardUp(Ptr<Packet> packet,
        ...
        if (m_state == ESTABLISHED && !(tcpHeader.GetFlags() & TcpHeader::RST)) {
            ...
            if (tcpHeader.GetFlags() & TcpHeader::CWR) {
                // Check if a packet with CE bit set is received. If there is no CE bit set, then
                // change the state to ECN_IDLE to stop sending ECN Echo messages. If there is CE
                // bit set, the packet should continue sending ECN Echo messages
                //
        ...
        if (m_rWnd.Get() == 0 &&
            m_persistEvent
                .IsExpired()) { // Zero window: Enter persist state to send 1 byte to probe
        ...
        case CLOSED:
            ...
            if ((tcpHeader.GetFlags() & ~(TcpHeader::PSH | TcpHeader::URG)) !=
                TcpHeader::RST) { // Since m_endPoint is not configured yet, we cannot use SendRST
                                  // here
                TcpHeader h;
        ...
        if (m_rWnd.Get() != 0 &&
            m_persistEvent
                .IsPending()) { // persist probes end, the other end has increased the window
    ...
    bool TcpSocketBase::IsTcpOptionEnabled(uint8_t kind) const {
    ...
    void TcpSocketBase::ReadOptions(const TcpHeader& tcpHeader, uint32_t* bytesSacked) {
        NS_LOG_FUNCTION(this << tcpHeader);
    ...
    void TcpSocketBase::EnterCwr(uint32_t currentDelivered) {
        ...
        if (!m_congestionControl->HasCongControl()) {
            ...
            NS_LOG_INFO("Enter CWR recovery mode; set cwnd to "
                        << m_tcb->m_cWnd << ", ssthresh to " << m_tcb->m_ssThresh << ", recover to "
                        << m_recover);
        }
    }

    void TcpSocketBase::EnterRecovery(uint32_t currentDelivered) {
        NS_LOG_FUNCTION(this);
        ...
        if (!m_congestionControl->HasCongControl()) {
            ...
            NS_LOG_INFO(m_dupAckCount << " dupack. Enter fast recovery mode."
                                      ...
                                      << m_tcb->m_ssThresh << " at fast recovery seqnum "
                                      << m_recover << " calculated in flight: " << bytesInFlight);
        }
    ...
    void TcpSocketBase::DupAck(uint32_t currentDelivered) {
        NS_LOG_FUNCTION(this);
    ...
    void TcpSocketBase::ReceivedAck(Ptr<Packet> packet, const TcpHeader& tcpHeader) {
        ...
        // Update bytes in flight before processing the ACK for proper calculation of congestion
        // window
        ...
        if (m_congestionControl->HasCongControl()) {
            ...
            uint32_t lost = (currentLost > previousLost) ? currentLost - previousLost
    ...
    void TcpSocketBase::ProcessAck(const SequenceNumber32& ackNumber,
                                   bool scoreboardUpdated,
                                   uint32_t currentDelivered,
                                   const SequenceNumber32& oldHeadSequence,
                                   bool receivedData) {
        NS_LOG_FUNCTION(this << ackNumber << scoreboardUpdated << currentDelivered
                             << oldHeadSequence);
         ...
         *
         * The check below implements conditions a), b), and d), and c) is prevented by virtue of
         * not reaching this code if SYN or FIN is set, and e) is not supported.
         */
        ...
        NS_LOG_DEBUG("ACK of " << ackNumber << " SND.UNA=" << oldHeadSequence
                               << " SND.NXT=" << m_tcb->m_nextTxSequence << " in state: "
        ...
        if (ackNumber == oldHeadSequence && ackNumber == m_tcb->m_highTxMark) {
            ...
            // of RecoveryPoint.
                ...
                if (m_tcb->m_congState == TcpSocketState::CA_OPEN) {
                    ...
                    NS_LOG_LOGIC("Congestion control called: cWnd: "
                                 << m_tcb->m_cWnd << " ssTh: " << m_tcb->m_ssThresh
                                 << " segsAcked: " << segsAcked);
    ...
    void TcpSocketBase::ProcessListen(Ptr<Packet> packet,
    ...
    void TcpSocketBase::ProcessSynSent(Ptr<Packet> packet, const TcpHeader& tcpHeader) {
        ...
        if (tcpflags == 0) { // Bare data, accept it and move to ESTABLISHED state. This is not a
            ...
            Simulator::ScheduleNow(&TcpSocketBase::ConnectionSucceeded, this);
                   ...
                   !(tcpflags & TcpHeader::SYN)) { // Ignore ACK in SYN_SENT
        } else if (tcpflags & TcpHeader::SYN &&
                   !(tcpflags & TcpHeader::ACK)) { // Received SYN, move to SYN_RCVD state and
            ...
            m_tcb->m_rxBuffer->SetNextRxSequence(tcpHeader.GetSequenceNumber() +
                                                 SequenceNumber32(1));
            /* Check if we received an ECN SYN packet. Change the ECN state of receiver to ECN_IDLE
             * if the traffic is ECN capable and sender has sent ECN SYN packet
             */

            if (m_tcb->m_useEcn != TcpSocketState::Off &&
                (tcpflags & (TcpHeader::CWR | TcpHeader::ECE)) ==
                ...
                SendEmptyPacket(TcpHeader::SYN | TcpHeader::ACK);
                   ...
                   m_tcb->m_nextTxSequence + SequenceNumber32(1) ==
                       tcpHeader.GetAckNumber()) { // Handshake completed
            NS_LOG_DEBUG("SYN_SENT -> ESTABLISHED");
            m_congestionControl->CongestionStateSet(m_tcb, TcpSocketState::CA_OPEN);
            m_tcb->m_congState = TcpSocketState::CA_OPEN;
            m_state = ESTABLISHED;
            ...
            /* Check if we received an ECN SYN-ACK packet. Change the ECN state of sender to
             * ECN_IDLE if receiver has sent an ECN SYN-ACK packet and the  traffic is ECN Capable
             */
            if (m_tcb->m_useEcn != TcpSocketState::Off &&
    ...
    void TcpSocketBase::ProcessSynRcvd(Ptr<Packet> packet,
                                       const TcpHeader& tcpHeader,
                                       const Address& fromAddress,
                                       const Address& /* toAddress */) {
        NS_LOG_FUNCTION(this << tcpHeader);
        ...
        if (tcpflags == 0 || (tcpflags == TcpHeader::ACK &&
                              m_tcb->m_nextTxSequence + SequenceNumber32(1) ==
                                  tcpHeader.GetAckNumber())) { // If it is bare data, accept it and
        ...
        } else if (tcpflags == TcpHeader::SYN) { // Probably the peer lost my SYN+ACK
            m_tcb->m_rxBuffer->SetNextRxSequence(tcpHeader.GetSequenceNumber() +
                                                 SequenceNumber32(1));
            /* Check if we received an ECN SYN packet. Change the ECN state of receiver to ECN_IDLE
             * if sender has sent an ECN SYN packet and the  traffic is ECN Capable
             */
            if (m_tcb->m_useEcn != TcpSocketState::Off &&
            ...
            if (tcpHeader.GetSequenceNumber() ==
                m_tcb->m_rxBuffer->NextRxSequence()) { // In-sequence FIN before connection
                                                       // complete. Set up connection and close.
                m_connected = true;
                m_retxEvent.Cancel();
                m_tcb->m_highTxMark = ++m_tcb->m_nextTxSequence;
                m_txBuffer->SetHeadSequence(m_tcb->m_nextTxSequence);
        ...
        if (packet->GetSize() > 0 && !(tcpflags & TcpHeader::ACK)) { // Bare data, accept it
            ...
            if (m_state == FIN_WAIT_1 && m_txBuffer->Size() == 0 &&
                tcpHeader.GetAckNumber() ==
                    m_tcb->m_highTxMark +
                        SequenceNumber32(1)) { // This ACK corresponds to the FIN sent
    ...
    void TcpSocketBase::ProcessClosing(Ptr<Packet> packet, const TcpHeader& tcpHeader) {
        ...
        if (tcpflags == TcpHeader::ACK) {
            if (tcpHeader.GetSequenceNumber() ==
                m_tcb->m_rxBuffer->NextRxSequence()) { // This ACK corresponds to the FIN sent
                TimeWait();
            }
        } else { // CLOSING state means simultaneous close, i.e. no one is sending data to
            // anyone. If anything other than ACK is received, respond with a reset.
            if (tcpflags == TcpHeader::FIN ||
                tcpflags ==
                    (TcpHeader::FIN |
                     TcpHeader::ACK)) { // FIN from the peer as well. We can close immediately.
                SendEmptyPacket(TcpHeader::ACK);
            } else if (tcpflags !=
                       TcpHeader::RST) { // Receive of SYN or SYN+ACK or bad flags or pure data
                NS_LOG_LOGIC("Illegal flag " << TcpHeader::FlagsToString(tcpflags)
                                             << " received. Reset packet is sent.");
                SendRST();
            }
    ...
    void TcpSocketBase::ProcessLastAck(Ptr<Packet> packet, const TcpHeader& tcpHeader) {
        ...
        if (tcpflags == 0) {
            ...
            if (tcpHeader.GetSequenceNumber() ==
                m_tcb->m_rxBuffer->NextRxSequence()) { // This ACK corresponds to the FIN sent. This
    ...
    void TcpSocketBase::DoPeerClose() {
        ...
        if (m_shutdownSend) { // The application declares that it would not sent any more, close
                              // this socket
            Close();
        } else { // Need to ack, the application will close later
            SendEmptyPacket(TcpHeader::ACK);
        }
        if (m_state == LAST_ACK) {
            ...
            Time lastRto =
                m_rtt->GetEstimate() + Max(m_clockGranularity, m_rtt->GetVariation() * 4);
    ...
    void TcpSocketBase::SendEmptyPacket(uint8_t flags) {
        ...
        if (hasSyn) {
            if (m_winScalingEnabled) { // The window scaling option is set only on SYN packets
            ...
            } else { // This is SYN retransmission
        ...
        if (flags & TcpHeader::ACK) { // If sending an ACK, cancel the delay ACK as well
    ...
    void TcpSocketBase::DeallocateEndPoint() {
    ...
    void TcpSocketBase::CompleteFork(Ptr<Packet> p [[maybe_unused]],
                                     const TcpHeader& h,
                                     const Address& fromAddress,
                                     const Address& toAddress) {
         ...
         */
        if (m_tcb->m_useEcn != TcpSocketState::Off &&
            (h.GetFlags() & (TcpHeader::CWR | TcpHeader::ECE)) ==
                (TcpHeader::CWR | TcpHeader::ECE)) {
            SendEmptyPacket(TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE);
            NS_LOG_DEBUG(TcpSocketState::EcnStateName[m_tcb->m_ecnState] << " -> ECN_IDLE");
            m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
    ...
    void TcpSocketBase::ConnectionSucceeded() { // Wrapper to protected function
                                                // NotifyConnectionSucceeded() so that it can
    ...
    void TcpSocketBase::AddSocketTags(const Ptr<Packet>& p, bool isEct) const {
        /*
    ...
    uint32_t TcpSocketBase::SendDataPacket(SequenceNumber32 seq, uint32_t maxSize, bool withAck) {
        ...
        NS_ASSERT(isRetransmission || ((m_highRxAckMark + SequenceNumber32(m_rWnd)) >=
                                       (seq + SequenceNumber32(maxSize))));
        ...
        if (m_closeOnEmpty && (remainingData == 0)) {
            ...
            if (m_state == ESTABLISHED) { // On active close: I am the first one to send FIN
                NS_LOG_DEBUG("ESTABLISHED -> FIN_WAIT_1");
                m_state = FIN_WAIT_1;
            } else if (m_state == CLOSE_WAIT) { // On passive close: Peer sent me FIN already
                NS_LOG_DEBUG("CLOSE_WAIT -> LAST_ACK");
                m_state = LAST_ACK;
            }
        }
        ...
        if (m_endPoint) {
            m_tcp->SendPacket(p,
                              ...
                              m_endPoint->GetPeerAddress(),
                              m_boundnetdevice);
            NS_LOG_DEBUG("Send segment of size " << sz << " with remaining data " << remainingData
                                                 << " via TcpL4Protocol to "
                                                 << m_endPoint->GetPeerAddress() << ". Header "
                                                 << header);
        } else {
            m_tcp->SendPacket(p,
                              header,
            ...
            NS_LOG_DEBUG("Send segment of size " << sz << " with remaining data " << remainingData
                                                 << " via TcpL4Protocol to "
                                                 << m_endPoint6->GetPeerAddress() << ". Header "
                                                 << header);
        }
    ...
    void TcpSocketBase::UpdateRttHistory(const SequenceNumber32& seq,
                                         uint32_t sz,
                                         bool isRetransmission) {
        NS_LOG_FUNCTION(this);

        // update the history of sequence numbers used to calculate the RTT
        if (!isRetransmission) { // This is the next expected one, just log at end
    ...
    uint32_t TcpSocketBase::SendPendingData(bool withAck) {
        ...
        while (availableWindow > 0) {
            ...
            if (!m_txBuffer->NextSeg(&next, &nextHigh, enableRule3)) {
                ...
                NS_LOG_LOGIC(" rxwin " << m_rWnd << " segsize " << m_tcb->m_segmentSize
                                       ...
                                       << " pd->Size " << m_txBuffer->Size() << " pd->SFS "
    ...
    uint32_t TcpSocketBase::UnAckDataCount() const {
        return m_tcb->m_highTxMark - m_txBuffer->HeadSequence();
    }

    uint32_t TcpSocketBase::BytesInFlight() const {
    ...
    uint16_t TcpSocketBase::AdvertisedWindowSize(bool scale) const {
        ...
        if (m_tcb->m_rxBuffer->GotFin()) {
            ...
            NS_ASSERT_MSG(
                m_tcb->m_rxBuffer->MaxRxSequence() - m_tcb->m_rxBuffer->NextRxSequence() >= 0,
    ...
    void TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader) {
        ...
        if (!m_tcb->m_rxBuffer->Add(p, tcpHeader)) { // Insert failed: No data or RX buffer full
            if (m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD ||
                m_tcb->m_ecnState == TcpSocketState::ECN_SENDING_ECE) {
                SendEmptyPacket(TcpHeader::ACK | TcpHeader::ECE);
                NS_LOG_DEBUG(TcpSocketState::EcnStateName[m_tcb->m_ecnState]
        ...
        if (expectedSeq <
            m_tcb->m_rxBuffer
                ->NextRxSequence()) { // NextRxSeq advanced, we have something to send to the app
            if (!m_shutdownRecv) {
                NotifyDataRecv();
            }
            // Handle exceptions
        ...
        if (m_tcb->m_rxBuffer->Size() > m_tcb->m_rxBuffer->Available() ||
            m_tcb->m_rxBuffer->NextRxSequence() >
                expectedSeq +
                    p->GetSize()) { // A gap exists in the buffer, or we filled a gap: Always ACK
        ...
        } else { // In-sequence packet: ACK if delayed ack count allows
            if (++m_delAckCount >= m_delAckMaxCount) {
                ...
                NS_LOG_LOGIC(this << " scheduled delayed ACK at "
                                  << (Simulator::Now() + Simulator::GetDelayLeft(m_delAckEvent))
                                         .GetSeconds());
            }
        }
    }

    Time TcpSocketBase::CalculateRttSample(const TcpHeader& tcpHeader,
    ...
    void TcpSocketBase::NewAck(const SequenceNumber32& ack, bool resetRTO) {
        ...
        if (m_state != SYN_RCVD &&
            resetRTO) { // Set RTO unless the ACK is received in SYN_RCVD state
            NS_LOG_LOGIC(
                this << " Cancelled ReTxTimeout event which was set to expire at "
                     << (Simulator::Now() + Simulator::GetDelayLeft(m_retxEvent)).GetSeconds());
            m_retxEvent.Cancel();
        ...
        if (m_txBuffer->Size() == 0 && m_state != FIN_WAIT_1 &&
            m_state != CLOSING) { // No retransmit timer if no data to retransmit
            NS_LOG_LOGIC(
                this << " Cancelled ReTxTimeout event which was set to expire at "
                     << (Simulator::Now() + Simulator::GetDelayLeft(m_retxEvent)).GetSeconds());
            m_retxEvent.Cancel();
    ...
    void TcpSocketBase::ReTxTimeout() {
        ...
        if (m_txBuffer->Size() == 0) {
            if (m_state == FIN_WAIT_1 || m_state == CLOSING) { // Must have lost FIN, re-send
                SendEmptyPacket(TcpHeader::FIN);
            }
            return;
        }
        ...
        NS_LOG_DEBUG("RTO. Reset cwnd to " << m_tcb->m_cWnd << ", ssthresh to " << m_tcb->m_ssThresh
                                           ...
                                           << " doubled rto to " << m_rto.Get().GetSeconds()
                                           << " s");
    ...
    void TcpSocketBase::DelAckTimeout() {
        m_delAckCount = 0;
        m_congestionControl->CwndEvent(m_tcb, TcpSocketState::CA_EVENT_DELAYED_ACK);
        if (m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD ||
            m_tcb->m_ecnState == TcpSocketState::ECN_SENDING_ECE) {
    ...
    void TcpSocketBase::LastAckTimeout() {
        NS_LOG_FUNCTION(this);
    ...
    void TcpSocketBase::TimeWait() {
        ...
        m_timewaitEvent =
            Simulator::Schedule(Seconds(2 * m_msl), &TcpSocketBase::CloseAndNotify, this);
    }
    ...
    uint32_t TcpSocketBase::GetSndBufSize() const {
    ...
    uint32_t TcpSocketBase::GetRcvBufSize() const {
    ...
    uint32_t TcpSocketBase::GetSegSize() const {
    ...
    Time TcpSocketBase::GetConnTimeout() const {
        return m_cnTimeout;
    }
    ...
    uint32_t TcpSocketBase::GetSynRetries() const {
        return m_synRetries;
    }
    ...
    uint32_t TcpSocketBase::GetDataRetries() const {
        NS_LOG_FUNCTION(this);
    ...
    Time TcpSocketBase::GetDelAckTimeout() const {
    ...
    uint32_t TcpSocketBase::GetDelAckMaxCount() const {
    ...
    bool TcpSocketBase::GetTcpNoDelay() const {
        return m_noDelay;
    }
    ...
    Time TcpSocketBase::GetPersistTimeout() const {
    ...
    bool TcpSocketBase::GetAllowBroadcast() const {
        return false;
    }
    ...
    void TcpSocketBase::ProcessOptionWScale(const Ptr<const TcpOption> option) {
        ...
        NS_LOG_INFO(m_node->GetId()
                    << " Received a scale factor of " << static_cast<int>(m_sndWindShift));
    }

    uint8_t TcpSocketBase::CalculateWScale() const {
        NS_LOG_FUNCTION(this);
    ...
    void TcpSocketBase::AddOptionWScale(TcpHeader& header) {
        ...
        NS_LOG_INFO(m_node->GetId()
                    << " Send a scaling factor of " << static_cast<int>(m_rcvWindShift));
    }

    uint32_t TcpSocketBase::ProcessOptionSack(const Ptr<const TcpOption> option) {
        NS_LOG_FUNCTION(this << option);
    ...
    void TcpSocketBase::ProcessOptionSackPermitted(const Ptr<const TcpOption> option) {
        NS_LOG_FUNCTION(this << option);
    ...
    void TcpSocketBase::AddOptionSackPermitted(TcpHeader& header) {
        NS_LOG_FUNCTION(this << header);
        NS_ASSERT(header.GetFlags() & TcpHeader::SYN);
    ...
    void TcpSocketBase::AddOptionSack(TcpHeader& header) {
        NS_LOG_FUNCTION(this << header);
    ...
    void TcpSocketBase::ProcessOptionTimestamp(const Ptr<const TcpOption> option,
        ...
        NS_LOG_INFO(m_node->GetId()
                    << " Got timestamp=" << m_timestampToEcho << " and Echo=" << ts->GetEcho());
    }
    ...
    Time TcpSocketBase::GetMinRto() const {
        return m_minRto;
    }
    ...
    Time TcpSocketBase::GetClockGranularity() const {
        return m_clockGranularity;
    }

    Ptr<TcpTxBuffer> TcpSocketBase::GetTxBuffer() const {
        return m_txBuffer;
    }

    Ptr<TcpRxBuffer> TcpSocketBase::GetRxBuffer() const {
        return m_tcb->m_rxBuffer;
    }

    void TcpSocketBase::SetRetxThresh(uint32_t retxThresh) {
        m_retxThresh = retxThresh;
        m_txBuffer->SetDupAckThresh(retxThresh);
    }

    void TcpSocketBase::UpdatePacingRateTrace(DataRate oldValue, DataRate newValue) const {
        m_pacingRateTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateCwnd(uint32_t oldValue, uint32_t newValue) const {
        m_cWndTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateCwndInfl(uint32_t oldValue, uint32_t newValue) const {
        m_cWndInflTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateSsThresh(uint32_t oldValue, uint32_t newValue) const {
    ...
    void TcpSocketBase::UpdateEcnState(TcpSocketState::EcnState_t oldValue,
                                       TcpSocketState::EcnState_t newValue) const {
        m_ecnStateTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateNextTxSequence(SequenceNumber32 oldValue,
    ...
    void TcpSocketBase::UpdateHighTxMark(SequenceNumber32 oldValue,
                                         SequenceNumber32 newValue) const {
        m_highTxMarkTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateBytesInFlight(uint32_t oldValue, uint32_t newValue) const {
        m_bytesInFlightTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateRtt(Time oldValue, Time newValue) const {
        m_srttTrace(oldValue, newValue);
    }

    void TcpSocketBase::UpdateLastRtt(Time oldValue, Time newValue) const {
    ...
    void TcpSocketBase::SetRecoveryAlgorithm(Ptr<TcpRecoveryOps> recovery) {
        NS_LOG_FUNCTION(this << recovery);
        m_recoveryOps = recovery;
    }

    Ptr<TcpSocketBase> TcpSocketBase::Fork() {
        return CopyObject<TcpSocketBase>(this);
    }

    uint32_t TcpSocketBase::SafeSubtraction(uint32_t a, uint32_t b) {
        if (a > b) {
            return a - b;
        }
    ...
    void TcpSocketBase::NotifyPacingPerformed() {
        NS_LOG_FUNCTION(this);
        NS_LOG_INFO("Performing Pacing");
        SendPendingData(m_connected);
    }

    bool TcpSocketBase::IsPacingEnabled() const {
        if (!m_tcb->m_pacing) {
            return false;
        } else {
    ...
    void TcpSocketBase::UpdatePacingRate() {
    ...
    void TcpSocketBase::SetPacingStatus(bool pacing) {
        NS_LOG_FUNCTION(this << pacing);
        m_tcb->m_pacing = pacing;
    }

    void TcpSocketBase::SetPaceInitialWindow(bool paceWindow) {
        NS_LOG_FUNCTION(this << paceWindow);
        m_tcb->m_paceInitialWindow = paceWindow;
    }

    bool TcpSocketBase::IsEct(TcpPacketType_t packetType) const {
    ...
    void TcpSocketBase::SetUseEcn(TcpSocketState::UseEcn_t useEcn) {
        NS_LOG_FUNCTION(this << useEcn);
        m_tcb->m_useEcn = useEcn;
    }

    uint32_t TcpSocketBase::GetRWnd() const {
        return m_rWnd.Get();
    }

    SequenceNumber32 TcpSocketBase::GetHighRxAck() const {
