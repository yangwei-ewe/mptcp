...
namespace ns3 {
    ...
    class MpTcpSocketBase : public TcpSocketBase {
      public: // public methods
        ...
        // uint32_t Recv(uint8_t* buf, uint32_t size); // Receive data from receiveing buffer -
        // TcpRxBuffe API need to be used in future!
        uint32_t Recv(
        ...
        // SetRcvBufSize() is implemented instead!
        ...
        void SetCongestionCtrlAlgo(
            CongestionCtrl_t ccalgo); // This would be used by attribute system
        // for setting congestion control
        void SetDataDistribAlgo(DataDistribAlgo_t ddalgo); // Round Robin is only algorithms used.
        void SetPathManager(PathPolicy);
        virtual std::size_t SetLocalAddress(const vector<MpTcpAddressInfo>& address);
      ...
      protected: // protected methods
        ...
        int SetupEndpoint(void); // Configure local address for given remote address in a host - it
        // query a routing protocol to find a source
        void CompleteFork(Ptr<Packet> p,
                          const TcpHeader& h,
                          const Address& fromAddress,
                          const Address& toAddress);
        void AdvertiseAvailableAddresses(); // Advertise all addresses to the peer, including the
        ...
        virtual void DoForwardUp(Ptr<Packet> p,
                                 Ipv4Header header,
                                 uint16_t port,
                                 Ptr<Ipv4Interface> interface);
        ...
        void ProcessListen(uint8_t sFlowIdx,
                           Ptr<Packet>,
                           const TcpHeader&,
                           const Address&,
                           const Address&);
        ...
        void ProcessSynRcvd(uint8_t sFlowIdx,
                            Ptr<Packet>,
                            const TcpHeader&,
                            const Address&,
                            const Address&);
        ...
        void ProcessRST(uint8_t sFlowIdx, Ptr<const pkg_mp_tcprst> pkg);
        uint8_t ProcessOption(TcpOption* opt);

        // Window Management
        virtual uint32_t BytesInFlight(
            uint8_t sFlowIdx); // Return total bytes in flight of a subflow
        uint16_t AdvertisedWindowSize();
        uint32_t AvailableWindow(uint8_t sFlowIdx);

        // Manage data Tx/Rx
        virtual Ptr<TcpSocketBase> Fork(void);
        virtual void ReceivedAck(uint8_t sFlowIdx,
                                 Ptr<Packet>,
                                 const TcpHeader&); // Received an ACK packet
        ...
        virtual bool ReadOptions(uint8_t sFlowIdx,
                                 Ptr<Packet> pkt,
                                 const TcpHeader&); // Read option from incoming packets
        virtual bool ReadOptions(
            Ptr<Packet> pkt,
            const TcpHeader&); // Read option from incoming packets (Listening Socket only)
        virtual void DupAck(
            const TcpHeader& t,
            uint32_t count); // Not in operation, it's pure virtual function from TcpSocketBase
        virtual void DupAck(uint8_t sFlowIdx,
                            DSNMapping* ptrDSN); // Congestion control algorithms -> loss recovery
        ...
        uint8_t LookupByAddrs(
            Ipv4Address src,
            Ipv4Address dst); // Called by Forwardup() to find the right subflow for incoing packet
        virtual int LookupSubflow(Ipv4Address src,
                                  uint16_t sPort,
                                  Ipv4Address dst,
        ...
        bool IsThereRoute(
            Ipv4Address src,
            Ipv4Address dst); // Called by InitiateSubflow & LookupByAddrs and Connect to
        // check whether there is route between a pair of addresses.
        bool IsLocalAddress(Ipv4Address addr);
        bool IsRemoteAddress(Ipv4Address addr);
        void RemoveAddress(std::map<size_t, shared_ptr<MpTcpAddressInfo>>& addrs, uint8_t addr_id);
        static Ptr<NetDevice> NetDev_of(Ipv4Address,
                                        Ptr<Node>); // Find Netdevice object of specific IP address.
        ...
        void GeneratePlots();
        void IsLastAck();
        void UpdateConnetionAck(uint32_t ack);
      ...
      protected: // protected variables
        ...
        uint8_t maxSubflows;
        int16_t lastUsedsFlowIdx;

        // MPTCP containers
        // vector<Ptr<MpTcpSubFlow>> subflows;
        PathManager pathManager;
        // map<size_t, shared_ptr<MpTcpAddressInfo>> localAddrs;
        ...
        PathPolicy pachPolicy;         // Mechanism for subflow establishement
        ...
        // this setting will apply throughout the entire session.

        void closeSubflow(uint8_t sFlowIdx, bool fast_close);
        std::size_t NegotiateSubflow();
        size_t RegisterSubflow(size_t local_addr_id, size_t remote_addr_id);
        // vector<Ptr<MpTcpSocketBase>> m_socket; // to hold orphan
      private:
