...
namespace ns3 {
    ...
    Time RttEstimator::GetEstimate() const {
        return m_estimatedRtt;
    }

    Time RttEstimator::GetVariation() const {
    ...
    void RttEstimator::NotifyConstructionCompleted() {
        NS_LOG_FUNCTION(this);
    ...
    uint32_t RttEstimator::GetNSamples() const {
        return m_nSamples;
    }
    ...
    void RttMeanDeviation::IntegerUpdate(Time m, uint32_t rttShift, uint32_t variationShift) {
    ...
    void RttMeanDeviation::Measurement(Time m) {
        ...
        if (m_nSamples) {
            ...
            m_estimatedRtt = Max(m, MilliSeconds(10)); // Set estimate to current
            m_estimatedVariation = m_estimatedRtt / 2; // And variation to current / 2
