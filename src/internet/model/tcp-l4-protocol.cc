...
#define NS_LOG_APPEND_CONTEXT                                                                      \
        ...
        static TypeId tid =
            TypeId("ns3::TcpL4Protocol")
                ...
                .AddAttribute("CongestionType",
                              "Congestion algo of TCP objects.",
    ...
    int TcpL4Protocol::GetProtocolNumber() const {
        return PROT_NUMBER;
    }

    void TcpL4Protocol::DoDispose() {
        NS_LOG_FUNCTION(this);
        m_sockets.clear();
    ...
    Ptr<Socket> TcpL4Protocol::CreateSocket(TypeId congestionTypeId) {
        return CreateSocket(congestionTypeId, m_recoveryTypeId);
    }

    Ptr<Socket> TcpL4Protocol::CreateSocket(TypeId congestionTypeId, TypeId recoveryTypeId) {
        ...
        NS_LOG_INFO("making TcpCongestionOps");
    ...
    Ptr<Socket> TcpL4Protocol::CreateSocket() {
        return CreateSocket(m_congestionTypeId, m_recoveryTypeId);
    }

    Ipv4EndPoint* TcpL4Protocol::Allocate() {
        NS_LOG_FUNCTION(this);
        return m_endPoints->Allocate();
    }

    Ipv4EndPoint* TcpL4Protocol::Allocate(Ipv4Address address) {
        NS_LOG_FUNCTION(this << address);
        return m_endPoints->Allocate(address);
    }

    Ipv4EndPoint* TcpL4Protocol::Allocate(Ptr<NetDevice> boundNetDevice, uint16_t port) {
        NS_LOG_FUNCTION(this << boundNetDevice << port);
        return m_endPoints->Allocate(boundNetDevice, port);
    }

    Ipv4EndPoint* TcpL4Protocol::Allocate(Ptr<NetDevice> boundNetDevice,
                                          Ipv4Address address,
                                          uint16_t port) {
    ...
    Ipv4EndPoint* TcpL4Protocol::Allocate(Ptr<NetDevice> boundNetDevice,
                                          ...
                                          uint16_t peerPort) {
        NS_LOG_FUNCTION(this << boundNetDevice << localAddress << localPort << peerAddress
                             << peerPort);
    ...
    Ipv6EndPoint* TcpL4Protocol::Allocate6() {
        NS_LOG_FUNCTION(this);
        return m_endPoints6->Allocate();
    }

    Ipv6EndPoint* TcpL4Protocol::Allocate6(Ipv6Address address) {
        NS_LOG_FUNCTION(this << address);
        return m_endPoints6->Allocate(address);
    }

    Ipv6EndPoint* TcpL4Protocol::Allocate6(Ptr<NetDevice> boundNetDevice, uint16_t port) {
        NS_LOG_FUNCTION(this << boundNetDevice << port);
        return m_endPoints6->Allocate(boundNetDevice, port);
    }

    Ipv6EndPoint* TcpL4Protocol::Allocate6(Ptr<NetDevice> boundNetDevice,
                                           Ipv6Address address,
                                           uint16_t port) {
        NS_LOG_FUNCTION(this << boundNetDevice << address << port);
    ...
    void TcpL4Protocol::ReceiveIcmp(Ipv4Address icmpSource,
                                    uint8_t icmpTtl,
                                    uint8_t icmpType,
                                    uint8_t icmpCode,
                                    uint32_t icmpInfo,
        ...
        NS_LOG_FUNCTION(this << icmpSource << (uint16_t)icmpTtl << (uint16_t)icmpType
                             << (uint16_t)icmpCode << icmpInfo << payloadSource
                             << payloadDestination);
        uint16_t src;
        uint16_t dst;
        src = payload[0] << 8;
    ...
    void TcpL4Protocol::ReceiveIcmp(Ipv6Address icmpSource,
                                    uint8_t icmpTtl,
                                    uint8_t icmpType,
                                    uint8_t icmpCode,
                                    uint32_t icmpInfo,
    ...
    IpL4Protocol::RxStatus TcpL4Protocol::PacketReceived(Ptr<Packet> packet,
        ...
        NS_LOG_LOGIC("TcpL4Protocol "
                     << this << " receiving seq " << incomingTcpHeader.GetSequenceNumber()
                     << " ack " << incomingTcpHeader.GetAckNumber() << " flags "
                     << TcpHeader::FlagsToString(incomingTcpHeader.GetFlags()) << " data size "
                     << packet->GetSize());
    ...
    void TcpL4Protocol::NoEndPointsFound(const TcpHeader& incomingHeader,
    ...
    IpL4Protocol::RxStatus TcpL4Protocol::Receive(Ptr<Packet> packet,
    ...
    void TcpL4Protocol::SendPacketV4(Ptr<Packet> packet,
        ...
        NS_LOG_LOGIC("TcpL4Protocol " << this << " sending seq " << outgoing.GetSequenceNumber()
                                      ...
                                      << TcpHeader::FlagsToString(outgoing.GetFlags())
    ...
    void TcpL4Protocol::SendPacketV6(Ptr<Packet> packet,
                                     const TcpHeader& outgoing,
    ...
    void TcpL4Protocol::SendPacket(Ptr<Packet> pkt,
                                   const TcpHeader& outgoing,
                                   const Address& saddr,
                                   const Address& daddr,
                                   Ptr<NetDevice> oif) const {
    ...
    void TcpL4Protocol::AddSocket(Ptr<TcpSocketBase> socket) {
    ...
    bool TcpL4Protocol::RemoveSocket(Ptr<TcpSocketBase> socket) {
        NS_LOG_FUNCTION(this << socket);
    ...
    IpL4Protocol::DownTargetCallback TcpL4Protocol::GetDownTarget() const {
        return m_downTarget;
    }
    ...
    IpL4Protocol::DownTargetCallback6 TcpL4Protocol::GetDownTarget6() const {
