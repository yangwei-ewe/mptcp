...
namespace ns3 {
    ...
    TypeId MpTcpBulkSendApplication::GetTypeId(void) {
        static TypeId tid =
            TypeId("ns3::MpTcpBulkSendApplication")
                .SetParent<Application>()
                .AddConstructor<MpTcpBulkSendApplication>()
                .AddAttribute("SendSize",
                              "The amount of data to send each time from application buffer to "
                              "socket buffer.",
    ...
    void MpTcpBulkSendApplication::SetBuffer(uint32_t buffSize) {
    ...
    void MpTcpBulkSendApplication::SetMaxBytes(uint32_t maxBytes) {
        NS_LOG_FUNCTION(this << maxBytes);
        m_maxBytes = maxBytes;
    }

    Ptr<Socket> MpTcpBulkSendApplication::GetSocket(void) const {
        NS_LOG_FUNCTION(this);
        return m_socket;
    }

    void MpTcpBulkSendApplication::DoDispose(void) {
    ...
    {
        NS_LOG_FUNCTION(this);
        NS_LOG_DEBUG("Node: " << GetNode()->GetId() << "Remote: " << m_peer
                              << " MaxBytes: " << m_maxBytes << " SendSize: " << m_sendSize);
        // Create the socket if not already
        if (!m_socket) {
            m_socket = CreateObject<MpTcpSocketBase>(
                GetNode()); // m_socket = Socket::CreateSocket (GetNode (), m_tid);
            m_socket->Bind();
            int result = m_socket->Connect(m_peer);
            if (result == 0) {
                m_socket->SetConnectCallback(
                    MakeCallback(&MpTcpBulkSendApplication::ConnectionSucceeded, this),
                    MakeCallback(&MpTcpBulkSendApplication::ConnectionFailed, this));
                m_socket->SetDataSentCallback(
                    MakeCallback(&MpTcpBulkSendApplication::DataSend, this));
                // m_socket->SetSendCallback(MakeCallback(&MpTcpBulkSendApplication::DataSend,
                // this));
            } else {
    ...
    void MpTcpBulkSendApplication::SendData(void) {
        NS_LOG_FUNCTION(this);
        NS_LOG_DEBUG("m_totBytes: " << m_totBytes << " maxByte: " << m_maxBytes
                                    << " GetTxAvailable: " << m_socket->GetTxAvailable()
                                    << " SendSize: " << m_sendSize);

        while (((!m_maxBytes) || (m_totBytes < m_maxBytes)) &&
               m_socket->GetTxAvailable()) { // Time to send more new data into MPTCP socket buffer

            uint32_t tmp = std::min(m_sendSize, m_maxBytes - m_totBytes);
            uint32_t toSend = std::min(tmp, m_socket->GetTxAvailable());

            // int actual = m_socket->FillBuffer(&m_data[m_totBytes], toSend);
            int actual = m_socket->FillBuffer(stuffing, toSend);
            m_totBytes += actual;
